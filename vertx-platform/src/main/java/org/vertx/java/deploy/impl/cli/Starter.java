...
public class Starter {
  ...
  private void runVerticle(boolean module, String main, Args args) {
    ...
    if (clustered) {
      ...
      int advertisePort = args.getInt("-advertise-port");
      if (advertisePort == -1)
    	  advertisePort = clusterPort;
      ...
      String advertiseHost = args.map.get("-advertise-host");
      if (advertiseHost == null)
    	  advertiseHost = clusterHost;
      vertx = new DefaultVertx(clusterPort, clusterHost, advertisePort, advertiseHost);
    }
    String repo = args.map.get("-repo");
